    ...
    <section id="pricing" className="py-24">
      <div className="container mx-auto px-4">
          ...
          {PLANS.map((plan) => (
            <div
              ...
              className={`relative p-8 rounded-2xl border flex flex-col ${
                plan.popular ? "border-primary shadow-lg bg-primary/5" : "bg-background shadow-sm"
              }`}
            >
              {plan.popular && (
        ...
        <p className="text-center text-sm text-muted-foreground mt-8">
          All plans include unlimited embeds, lead capture, and 80+ languages.{" "}
          <span className="text-foreground font-medium">Save ~2 months with annual billing.</span>
        </p>
      </div>
    </section>
  );
