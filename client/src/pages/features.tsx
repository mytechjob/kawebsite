  ...
  return (
    <PageLayout>
      <SEO
        ...
        description="Explore Knowledge Agents features: train on any content, take real actions, deploy as a widget or chat page, analytics, 80+ languages, human handoff, and enterprise-grade security."
        path="/features"
      ...
      <PageHero
        eyebrow="Platform"
        title="Everything you need to build agents that get work done"
        description="Knowledge Agents combine accurate, grounded answers with real action-taking — so your customers get resolutions, not just replies. No code required."
      />

      {/* Training sources highlight */}
        ...
        <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h2 className="text-3xl font-bold font-display mt-2 mb-4">
              Bring your knowledge from anywhere
            </h2>
            <p className="text-muted-foreground text-lg mb-6">
              Upload files, paste text, or point us at a URL. Your agent is trained in seconds and uses
              Retrieval-Augmented Generation to keep every answer grounded in your content — with citations.
              Schedule re-crawls so answers never go stale.
            </p>
          ...
          <div className="bg-muted/30 border rounded-2xl p-6 shadow-sm">
            <div className="space-y-3">
              {["Help_Center.pdf", "yoursite.com/docs", "Returns_Policy.docx", "Product FAQ (pasted)"].map(
                (src, i) => (
                  <div key={src} className="flex items-center justify-between p-3 bg-background rounded-lg border">
      ...
      <section className="py-24">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-14">
            <span className="text-sm font-semibold uppercase tracking-wider text-primary">Deploy anywhere</span>
            <h2 className="text-3xl md:text-4xl font-bold font-display mb-4 mt-2">
              Three ways to put your agent to work
            </h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {deployOptions.map((opt) => {
              const Icon = opt.icon;
              return (
                <div key={opt.title} className="p-8 bg-background rounded-2xl border text-center">
                  <div className="w-14 h-14 mx-auto rounded-xl bg-primary/10 flex items-center justify-center mb-4">
                    <Icon className="w-7 h-7 text-primary" />
                  </div>
                  <h3 className="text-xl font-bold mb-2">{opt.title}</h3>
                  <p className="text-muted-foreground">{opt.text}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="py-16 bg-muted/30 border-y">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          ...
          <p className="text-muted-foreground text-lg">
            Data is encrypted in transit and at rest, scoped per workspace, and never used to train
            foundation models. Enterprise plans add SSO, audit logs, role-based access, and custom
