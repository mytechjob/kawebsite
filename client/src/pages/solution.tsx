    ...
    <PageLayout>
      <SEO
        ...
        jsonLd={[
          breadcrumbSchema([
            { name: "Home", path: "/" },
            { name: "Solutions", path: "/solutions/customer-support" },
        ...
        <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-sm font-medium mb-5">
              <Icon className="w-4 h-4" /> {solution.eyebrow}
            </div>
            <h1 className="text-4xl md:text-5xl font-bold font-display tracking-tight mb-5">
              {solution.headline}
            </h1>
          ...
          <div className="bg-background border rounded-2xl shadow-xl overflow-hidden">
            <div className="p-4 bg-primary text-primary-foreground flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center font-bold">KA</div>
              <div>
                <div className="font-bold text-sm">{solution.name} Agent</div>
                <div className="text-xs opacity-80">Online • answers instantly</div>
              </div>
            </div>
            <div className="p-5 space-y-4 bg-muted/10">
              {solution.sampleChat.map((m, i) => (
      ...
      <section className="py-16">
        <div className="container mx-auto px-4 max-w-3xl text-center">
          <p className="text-xl leading-relaxed text-foreground/80">{solution.intro}</p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {solution.challenges.map((c) => (
              <div key={c} className="flex items-start gap-3 p-5 bg-background rounded-xl border">
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-14">
            ...
            <h2 className="text-3xl md:text-4xl font-bold font-display mt-2">
              How Knowledge Agents help {solution.name.toLowerCase()} teams
            </h2>
          </div>
          <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
            {solution.capabilities.map((cap) => (
              <div key={cap.title} className="p-6 bg-background rounded-xl border">
        ...
        <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-12">
          <div>
            <span className="inline-flex items-center gap-1.5 text-sm font-semibold uppercase tracking-wider text-primary">
              <Sparkles className="w-4 h-4" /> Actions it can take
            </span>
            <h2 className="text-2xl md:text-3xl font-bold font-display mt-2 mb-6">
              Resolutions, not just replies
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-4 content-start">
            {solution.results.map((r) => (
              <div key={r.label} className="p-6 bg-background rounded-xl border text-center lg:text-left lg:flex lg:items-baseline lg:gap-3">
                <div className="text-3xl font-bold font-display text-primary">{r.value}</div>
