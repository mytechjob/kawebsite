...
import { ArrowRight, MessageSquare, Sparkles, BookOpen, LayoutTemplate, ShieldCheck } from "lucide-react";

const FAQS: Faq[] = [
  {
    q: "What is a Knowledge Agent in simple terms?",
    a: "A Knowledge Agent is an AI assistant you create by training it on your own content — like your help center, documents, and FAQs. It answers customer questions in natural language 24/7 and can take real actions such as booking meetings, creating support tickets, checking orders, and capturing leads. You deploy it as a chat widget on your website or as a standalone chat page with its own URL.",
  },
  {
    q: "How is a Knowledge Agent different from a chatbot?",
    a: "Traditional chatbots follow scripted decision trees and can only handle pre-programmed questions. A Knowledge Agent understands natural language, grounds its answers in your real content using Retrieval-Augmented Generation (RAG) with citations, and can take actions by securely calling your tools and APIs — not just reply with text.",
  },
  {
    q: "Do I need technical skills to build a Knowledge Agent?",
    a: "No. Building one is no-code: you upload content or crawl a website, customize the look and behavior, and embed it. Developers can optionally use the API and custom actions for advanced builds.",
  },
  {
    q: "What can a Knowledge Agent be trained on?",
    a: "PDF, DOCX, TXT, and Markdown files, pasted text, entire websites and help centers via crawling, sitemaps, and tools like Notion. You can combine multiple sources into one agent and re-crawl on a schedule to stay current.",
  },
  {
    q: "Where can I use a Knowledge Agent?",
    a: "Embed it as a chat widget on any website with one script tag, publish it as a standalone chat page to share via link or QR code, or integrate it into your product through the API.",
  },
];

function TocLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a href={href} className="block py-1.5 text-sm text-muted-foreground hover:text-primary transition-colors">
      {children}
    </a>
  );
}
  ...
  return (
    <PageLayout>
      <SEO
        ...
        path="/what-is-a-knowledge-agent"
        keywords={[
          "what is a knowledge agent",
          "knowledge agent definition",
          "AI knowledge agent",
          "knowledge agent vs chatbot",
        ...
        jsonLd={[
          breadcrumbSchema([
            { name: "Home", path: "/" },
            { name: "What is a Knowledge Agent?", path: "/what-is-a-knowledge-agent" },
          ]),
          faqSchema(FAQS),
          {
            ...
            description:
              "An AI assistant trained on an organization's own content that answers customer questions 24/7 and takes real actions such as booking meetings, creating tickets, checking orders, and capturing leads. It can be deployed as a website chat widget or a standalone chat page.",
            inDefinedTermSet: absoluteUrl("/what-is-a-knowledge-agent"),
          },
        ]}
      />
      ...
      <article className="py-12 md:py-16">
        <div className="container mx-auto px-4 max-w-3xl">
          ...
          <h1 className="text-3xl md:text-5xl font-bold font-display tracking-tight mt-2 mb-6">
            What is a Knowledge Agent?
          </h1>

          {/* Direct-answer definition box — optimized for AI answer engines */}
          <div className="p-6 rounded-2xl border-2 border-primary/20 bg-primary/5 mb-10">
            <p className="text-lg leading-relaxed">
              A <strong>Knowledge Agent</strong> is an AI assistant you build by training it on your
              own content — your documents, website, help center, and FAQs. Unlike a basic chatbot, it
              answers customer questions in natural language <strong>24/7</strong> and can{" "}
              <strong>take real actions</strong> on the customer's behalf, such as booking meetings,
              creating support tickets, checking order status, and capturing leads. You can deploy it
              as a chat widget on your website or as a standalone, shareable chat page.
            </p>
          </div>
          ...
          <div className="space-y-5 text-lg leading-relaxed text-foreground/80">
            <h2 id="how-they-work" className="text-2xl md:text-3xl font-bold font-display text-foreground pt-6 scroll-mt-28">
              How Knowledge Agents work
            </h2>
            <p>
              Knowledge Agents are powered by large language models (LLMs) combined with a technique
              called <strong>Retrieval-Augmented Generation (RAG)</strong>. When you add content, it's
              split into chunks and indexed. When a customer asks a question, the agent retrieves the
              most relevant passages from <em>your</em> content and uses an LLM to compose an accurate,
              grounded answer — with citations back to the source.
            </p>
            <p>
            ...
            <h2 id="two-forms" className="text-2xl md:text-3xl font-bold font-display text-foreground pt-6 scroll-mt-28">
              The two forms: chat widget and chat page
            </h2>
            <div className="grid sm:grid-cols-2 gap-4 not-prose">
              <div className="p-5 rounded-xl border bg-background">
                <LayoutTemplate className="w-6 h-6 text-primary mb-2" />
                <h3 className="font-bold text-foreground text-base mb-1">Chat widget</h3>
                <p className="text-base">An embeddable chat bubble that lives on any website, added with a single script tag.</p>
              </div>
              <div className="p-5 rounded-xl border bg-background">
                <MessageSquare className="w-6 h-6 text-primary mb-2" />
                <h3 className="font-bold text-foreground text-base mb-1">Chat page</h3>
                <p className="text-base">A standalone, shareable page with its own URL — no website required. Perfect for links, QR codes, and bios.</p>
              </div>
            </div>

            <h2 id="answer-and-act" className="text-2xl md:text-3xl font-bold font-display text-foreground pt-6 scroll-mt-28">
              Answering questions <em>and</em> taking action
            </h2>
            <p>
              The defining feature of a Knowledge Agent is that it does more than answer. Through{" "}
            ...
            <h2 id="vs-chatbot" className="text-2xl md:text-3xl font-bold font-display text-foreground pt-6 scroll-mt-28">
              Knowledge Agent vs. traditional chatbot
            </h2>
            <p>
              A traditional chatbot follows rigid, pre-scripted decision trees and breaks the moment a
            ...
            <h2 id="how-to-build" className="text-2xl md:text-3xl font-bold font-display text-foreground pt-6 scroll-mt-28">
              How to build a Knowledge Agent
            </h2>
            <p>
